...
namespace CSharpImageLibrary.DDS
{
    ...
    internal static class BC7
    {
        struct Mode
        {
            ...
            public readonly RGBAColour RawRGBPrecision;
            public readonly RGBAColour RGBPrecisionWithP;
            ...
            public Mode(int modeVal, int partitions, int partitionBits, int IndexPrecision, RGBAColour rawrgbPrecision, RGBAColour RGBPrecisionWithP, int APrecision, int PBits, int RotationBits, int IndexModeBits)
            {
        ...
        static Mode[] Modes = new Mode[] {
            /* Mode 0: */ new Mode(0, 2, 4, 3, new RGBAColour(4, 4, 4, 0), new RGBAColour(5, 5, 5, 0), 0, 6, 0, 0),
            /* Mode 1: */ new Mode(1, 1, 6, 3, new RGBAColour(6, 6, 6, 0), new RGBAColour(7, 7, 7, 0), 0, 2, 0, 0),
            /* Mode 2: */ new Mode(2, 2, 6, 2, new RGBAColour(5, 5, 5, 0), new RGBAColour(5, 5, 5, 0), 0, 0, 0, 0),
            /* Mode 3: */ new Mode(3, 1, 6, 2, new RGBAColour(7, 7, 7, 0), new RGBAColour(8, 8, 8, 0), 0, 4, 0, 0),
            /* Mode 4: */ new Mode(4, 0, 0, 2, new RGBAColour(5, 5, 5, 6), new RGBAColour(5, 5, 5, 6), 3, 0, 2, 1),
            /* Mode 5: */ new Mode(5, 0, 0, 2, new RGBAColour(7, 7, 7, 8), new RGBAColour(7, 7, 7, 8), 2, 0, 2, 0),
            /* Mode 6: */ new Mode(6, 0, 0, 4, new RGBAColour(7, 7, 7, 7), new RGBAColour(8, 8, 8, 8), 0, 2, 0, 0),
            /* Mode 7: */ new Mode(7, 1, 6, 2, new RGBAColour(5, 5, 5, 5), new RGBAColour(6, 6, 6, 6), 0, 4, 0, 0),
        };

        private static RGBAColour Interpolate(RGBAColour RGBAColour1, RGBAColour RGBAColour2, int wc, int wa, int wcPrec, int waPrec)
        {
            RGBAColour temp = InterpolateRGB(RGBAColour1, RGBAColour2, wc, wcPrec);
            temp.A = InterpolateA(RGBAColour1, RGBAColour2, wa, waPrec);
            return temp;
        }

        #region Decompression
        public static RGBAColour[] DecompressBC7(byte[] source, int sourceStart)
        {
            int start = 0;
            ...
            var outColours = new RGBAColour[NUM_PIXELS_PER_BLOCK];
            ...
            if (modeVal < 8)
            {
                ...
                RGBAColour[] c = new RGBAColour[6];
                RGBAColour RGBPrecision = mode.RawRGBPrecision;
                RGBAColour RGBPrecisionWithP = mode.RGBPrecisionWithP;
        ...
        {
            RGBAColourEndPointPair[][] AllEndPoints = new RGBAColourEndPointPair[BC7_MAX_SHAPES][];
            for(int i = 0; i < BC7_MAX_SHAPES; i++)
                AllEndPoints[i] = new RGBAColourEndPointPair[BC7_MAX_REGIONS];

            RGBAColour[] block = new RGBAColour[NUM_PIXELS_PER_BLOCK];
        ...
        static float Refine(int shape, int rotation, int indexMode, RGBAColourEndPointPair[][] AllEndPoints, Mode mode, RGBAColour[] block, byte[] destination, int destStart)
        {
            RGBAColourEndPointPair[] EndPoints = AllEndPoints[shape];
            
            RGBAColourEndPointPair[] OrgEndPoints = new RGBAColourEndPointPair[BC7_MAX_REGIONS];
            RGBAColourEndPointPair[] OptEndPoints = new RGBAColourEndPointPair[BC7_MAX_REGIONS];
        ...
        static void EmitBlock(Mode mode, byte[] destination, int destStart, int rotation, int indexMode, int shape, RGBAColourEndPointPair[] endPoints, int[] indicies, int[] alphaIndicies)
        {
            int i = 0;
            int startBit = 0;
        ...
        static void OptimiseEndPoints(Mode mode, int indexMode, RGBAColour[] block, float[] afOrgErr, RGBAColourEndPointPair[] aOrgEndPoints, RGBAColourEndPointPair[] aOptEndPoints, int shape)
        {
            RGBAColour[] pixels = new RGBAColour[NUM_PIXELS_PER_BLOCK];
        ...
        static void OptimiseOne(float fOrgErr, Mode mode, ref RGBAColourEndPointPair org, ref RGBAColourEndPointPair opt, int indexMode, RGBAColour[] block, int np)
        {
            float fOptErr = fOrgErr;
            opt = org; 
            RGBAColourEndPointPair new_a = new RGBAColourEndPointPair(), new_b = new RGBAColourEndPointPair(), newEndPoints = new RGBAColourEndPointPair();
            bool do_b;
        ...
        static unsafe void Exhaustive(Mode mode, int ch, RGBAColourEndPointPair endPoint, ref int A, ref int B, int prec, float fOrgErr, int indexMode, int np, RGBAColour[] block)
        {
            RGBAColourEndPointPair temp;
            if (fOrgErr == 0)
                return;
        ...
        static unsafe float PerturbOne(Mode mode, int ch, ref RGBAColourEndPointPair newEndPoints, ref RGBAColourEndPointPair oldEndPoints, float oldErr, bool do_b, int indexMode, RGBAColour[] block, int np)
        {
            RGBAColourEndPointPair temp = newEndPoints = oldEndPoints;
            float minErr = oldErr;
        ...
        static float MapColours(int indexMode, Mode mode, RGBAColourEndPointPair endPoints, int np, RGBAColour[] block, float minErr)
        {
            ...
            RGBAColour[] palette = new RGBAColour[BC7_MAX_INDICIES];
            float totalErr = 0;
        ...
        static void AssignIndicies(int shape, int indexMode, Mode mode, RGBAColourEndPointPair[] endPoints, int[] indicies, int[] alphaIndicies, float[] afTotalErr, RGBAColour[] block)
        {
            ...
            RGBAColour[][] palette = new RGBAColour[BC7_MAX_REGIONS][];
            for (int i = 0; i < BC7_MAX_REGIONS; i++)
                palette[i] = new RGBAColour[BC7_MAX_INDICIES];
        ...
        static void GeneratePaletteQuantized(RGBAColourEndPointPair endPoints, Mode mode, RGBAColour[] palette, int indexMode)
        {
            int indexPrecision = indexMode != 0 ? mode.APrecision : mode.IndexPrecision;
            int alphaIndexPrecision = indexMode != 0 ? mode.IndexPrecision : mode.APrecision;
            ...
            RGBAColour a = Unquantise(endPoints.A, mode.RGBPrecisionWithP);
            RGBAColour b = Unquantise(endPoints.B, mode.RGBPrecisionWithP);
        ...
        static RGBAColour Quantize(RGBAColour colour, RGBAColour precision)
        {
            RGBAColour q = new RGBAColour
            {
        ...
        static float RoughMSE(int modeVal, RGBAColourEndPointPair[][] allEndPoints, RGBAColour[] block, int shape, int indexMode)
        {
            RGBAColourEndPointPair[] EndPoints = allEndPoints[shape];
            ...
            ScRGBAColour[] Colour = new ScRGBAColour[block.Length];
            float factor = 255f;
            for (int i = 0; i < block.Length; i++)
                Colour[i] = new ScRGBAColour(block[i].R / factor, block[i].G / factor, block[i].B / factor, block[i].A / factor);
            ...
            for (int p = 0; p <= partitions; p++)
            {
                ...
                if (alphaPrecision == 0)
                {
                    ScRGBAColour[] MinMax= OptimiseRGBA_BC67(Colour, 4, np, pixelIndicies);
                    EndPoints[p].A = new RGBAColour(MinMax[0].R, MinMax[0].G, MinMax[0].B, MinMax[0].A);
                    EndPoints[p].B = new RGBAColour(MinMax[1].R, MinMax[1].G, MinMax[1].B, MinMax[1].A);
                }
                else
                {
                    int maxA = 0, minA = 255;
                    ...
                    ScRGBAColour[] MinMax = OptimiseRGB_BC67(Colour, 4, np, pixelIndicies);
                    EndPoints[p].A = new RGBAColour(MinMax[0].R, MinMax[0].G, MinMax[0].B, MinMax[0].A);
                    EndPoints[p].B = new RGBAColour(MinMax[1].R, MinMax[1].G, MinMax[1].B, MinMax[1].A);
        ...
        private static float ComputeError(RGBAColour pixelColour, RGBAColour[] palette, int indexPrecision, int alphaPrecision, int[] bestIdxs = null)
        {
