...
namespace CSharpImageLibrary
{
    ...
    public static class ImageEngine
    {
        ...
        public static async Task<ConcurrentBag<string>> BulkConvert<IMG, MIP>(IEnumerable<string> files, ImageFormats.ImageEngineFormatDetails destFormatDetails, bool useSourceFormat, string saveFolder,
            MipHandling saveMipType = MipHandling.Default, bool useSourceAsDestination = false, bool removeAlpha = false, IProgress<int> progressReporter = null) where IMG : ImageEngineImageBase<MIP>, new() where MIP : MipMapBase, new()
        {
            ...
            if (EnableThreading && supportsParallel)
                Failures = await DoBulkParallel<IMG, MIP>(files, destFormatDetails, saveFolder, saveMipType, useSourceAsDestination, removeAlpha, progressReporter);
            else
        ...
        static async Task<ConcurrentBag<string>> DoBulkParallel<IMG, MIP>(IEnumerable<string> files, ImageFormats.ImageEngineFormatDetails destFormatDetails, string saveFolder,
            MipHandling saveMipType = MipHandling.Default, bool useSourceAsDestination = false, bool removeAlpha = false, IProgress<int> progressReporter = null) where IMG : ImageEngineImageBase<MIP>, new() where MIP : MipMapBase, new()
        {
            ConcurrentBag<string> failures = new ConcurrentBag<string>();
        ...
        public static IMG MergeChannels<IMG, T>(
            MergeChannelsImage<IMG, T> blue, 
            MergeChannelsImage<IMG, T> green, 
            MergeChannelsImage<IMG, T> red, 
            MergeChannelsImage<IMG, T> alpha) where IMG : ImageEngineImageBase<T>, new() where T : MipMapBase, new()
        {
            ...
            if (EnableThreading)
            {
                var merger = new Action<int, MergeChannelsImage<IMG, T>>((start, channel) =>
                {
